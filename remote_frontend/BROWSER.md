...
Label 保留双栏 Task / Episode 选择、单机位默认视图、左侧 Episode 进度、底部操作栏。
数字 0 为五机位加 Ego 总览，数字键切换机位；总览只读。原始视角、含可见性的原始视角、修改后视角按原顺序轮换。
...
确认后依原规则跟踪到下一帧；只有跟踪实际更新下一帧时才取消该帧的确认。显示 Skeleton 时不能确认。
全部帧确认后才可提交。返回任务先询问是否放弃未确认修改，并释放任务。
