...
.tile{{position:relative;min-width:0;min-height:0;background:#101820;border:1px solid #1e2a34;border-radius:7px;overflow:hidden;display:flex;align-items:center;justify-content:center}}.tile img{{width:100%;height:100%;object-fit:contain}}.tile label{{position:absolute;left:8px;top:7px;background:#0009;padding:3px 7px;border-radius:4px;font:12px ui-monospace}}
...
<script>
const episodeId={encoded_id};let sessionId=null,info=null,mode='rgb',framePos=0,playing=false,timer=null,closed=false;
...
async function renderFrame(){{if(!info||!info.frames.length)return;const frame=info.frames[framePos];setupFrames();$('#grid').style.display=mode==='rgb'||mode==='manomesh'?'grid':'none';$('#single').style.display=mode==='pico'?'flex':'none';$('#cloud').style.display=mode==='pointcloud'?'block':'none';
if(mode==='manomesh'&&!frameAvailable(frame)){{$('#modeStatus').textContent='正在缓冲该同步帧…';return}}
if(mode==='rgb'||mode==='manomesh'){{const sources=info.sources.filter(s=>s.kind==='multiview').slice(0,6);$('#grid').innerHTML=sources.map(s=>`<div class="tile"><img src="${{mediaUrl(mode==='rgb'?s.id:s.camera,frame)}}"><label>${{s.label}}</label></div>`).join('')}}
else if(mode==='pico')$('#singleImage').src=mediaUrl('ego',frame);else{{try{{const r=await fetch(mediaUrl('cloud',frame));const cloud=await r.json();drawCloud(cloud.points||[])}}catch(e){{$('#modeStatus').textContent=e.message}}}}}}
...
function playTick(){{if(!playing||!info||!info.frames.length)return;const next=(framePos+1)%info.frames.length,frame=info.frames[next];if(!frameAvailable(frame)){{$('#modeStatus').textContent='后台渲染中，正在缓冲下一同步帧…';schedulePlay(30);return}}framePos=next;renderFrame();schedulePlay(1000/(info.fps||30))}}
