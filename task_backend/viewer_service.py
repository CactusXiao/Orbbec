...
.tile{{position:relative;min-width:0;min-height:0;background:#101820;border:1px solid #1e2a34;border-radius:7px;overflow:hidden;display:flex;align-items:center;justify-content:center}}.tile canvas.frame{{position:absolute;inset:0;width:100%;height:100%}}.tile label{{position:absolute;z-index:2;left:8px;top:7px;background:#0009;padding:3px 7px;border-radius:4px;font:12px ui-monospace}}
...
function ensureSixGrid(sources){{const signature=sources.map(s=>s.id).join('|');if(signature===gridSignature)return;gridSignature=signature;lastGridKey='';pendingGridKey='';$('#grid').innerHTML=sources.map(s=>`<div class="tile"><canvas class="frame"></canvas><label>${{s.label}}</label></div>`).join('')}}
...
async function renderSix(frame){{const sources=info.sources.filter(s=>s.kind==='multiview').slice(0,6);ensureSixGrid(sources);const key=`${{mode}}:${{frame}}`;if(key===lastGridKey||key===pendingGridKey)return true;pendingGridKey=key;const token=++gridRenderToken;const urls=sources.map(s=>mediaUrl(mode==='rgb'?s.id:s.camera,frame));try{{const images=await Promise.all(urls.map(preloadImage));if(token!==gridRenderToken)return false;const canvases=[...document.querySelectorAll('#grid canvas.frame')],d=devicePixelRatio||1;canvases.forEach((canvas,i)=>{{const r=canvas.getBoundingClientRect(),w=Math.max(1,Math.round(r.width*d)),h=Math.max(1,Math.round(r.height*d)),image=images[i],scale=Math.min(w/image.naturalWidth,h/image.naturalHeight),dw=image.naturalWidth*scale,dh=image.naturalHeight*scale;canvas.width=w;canvas.height=h;const context=canvas.getContext('2d',{{alpha:false}});context.fillStyle='#101820';context.fillRect(0,0,w,h);context.drawImage(image,(w-dw)/2,(h-dh)/2,dw,dh)}});lastGridKey=key;return true}}finally{{if(pendingGridKey===key)pendingGridKey=''}}}}
...
window.addEventListener('resize',()=>{{if(mode==='pointcloud')drawCloud(cloudPoints);else if(mode==='rgb'||mode==='manomesh'){{lastGridKey='';renderFrame()}}}});start();
</script></body></html>"""
