...
class ViewerSessionManager:
    ...
    def _prepare_initial(self, session: ViewerSession) -> None:
        try:
            ...
            with session.lock:
                session.sources = [s for s in sources if s.kind != "multiview"] + rgb_sources
                session.modes["rgb"] = ModeState(
                    "ready",
                    len(frames),
                    len(frames),
                    f"{len(rgb_sources)} 路 RGB 已就绪",
                    playable=True,
                    complete=True,
    ...
    def _prepare_pointcloud(self, session: ViewerSession, state: ModeState) -> None:
        ...
        sources = [source for source in session.sources if source.kind == "multiview"]
        if not sources or any(self._depth_frame_path(source, frames[0], required=False) is None for source in sources):
            raise ViewerError("彩色融合点云需要各机位已解码的 RGB 与 16 位 Depth")
    ...
    def _build_rgbd_color_cloud(self, session: ViewerSession, frame: int, *, max_points: int) -> Any:
        ...
        step = 2
        max_depth_m = 6.0
        for source in [item for item in session.sources if item.kind == "multiview"]:
    ...
    def _prepare_manomesh(self, session: ViewerSession, state: ModeState) -> None:
        ...
        cameras = [s.camera for s in session.sources if s.kind == "multiview"]
        frames = list(session.frame_indices)
...
#stage{{position:relative;min-height:0;padding:14px;background:#080c10;overflow:hidden}}#grid{{height:100%;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));grid-template-rows:repeat(2,calc((100% - 8px)/2));grid-auto-rows:calc((100% - 8px)/2);gap:8px;overflow-y:auto;align-content:start}}
...
@media(max-width:850px){{.app{{grid-template-columns:190px 1fr}}#grid{{grid-template-columns:repeat(2,1fr);grid-template-rows:repeat(3,calc((100% - 16px)/3));grid-auto-rows:calc((100% - 16px)/3)}}}}
{TACTILE_CSS}
</style></head><body><div class="app"><aside><h1>Episode Viewer</h1><div class="sub" id="episodeLabel"></div>
<button class="mode active" data-mode="rgb" disabled>7 路 RGB</button><button class="mode" data-mode="pico" disabled>Pico + 眼动</button>
...
<div class="spacer"></div><div id="sessionState">正在创建临时会话…</div></aside><main><header><strong id="title">7 路 RGB</strong><span id="modeStatus"></span><span id="manoSource"></span></header>
...
function ensureCameraGrid(sources){{const signature=sources.map(s=>s.id).join('|');if(signature===gridSignature)return;gridSignature=signature;lastGridKey='';pendingGridKey='';$('#grid').innerHTML=sources.map(s=>`<div class="tile"><img class="frame active" alt=""><img class="frame" alt=""><label>${{s.label}}</label></div>`).join('')}}
...
if(mode==='rgb'||mode==='manomesh'){{try{{return await renderCameraGrid(frame)}}catch(e){{$('#modeStatus').textContent=e.message;return false}}}}
