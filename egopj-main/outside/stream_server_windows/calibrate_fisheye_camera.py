...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional fisheye .npz whose K and D are used only as optimizer initial values.",
    )
...
def calibrate(
    ...
    k = (
        np.asarray(initial_k, dtype=np.float64).reshape(3, 3).copy()
        if initial_k is not None
        else np.zeros((3, 3), dtype=np.float64)
    )
    d = (
        np.asarray(initial_d, dtype=np.float64).reshape(4, 1).copy()
...
def main() -> int:
    ...
    if args.initial_calibration is not None:
        initial_path = args.initial_calibration.resolve()
        if not initial_path.is_file():
            raise SystemExit(f"initial calibration not found: {initial_path}")
        with np.load(initial_path) as initial_data:
            initial_k = np.asarray(initial_data["K"], dtype=np.float64).reshape(3, 3)
            initial_d = np.asarray(initial_data["D"], dtype=np.float64).reshape(4, 1)
    objpoints = []
    imgpoints = []
    used_images = []
    rejected = []
    ...
    for image_path in images:
        ...
        if not ok or corners is None:
            ...
            continue
        debug = image.copy()
        cv2.drawChessboardCorners(debug, args.pattern, corners.astype(np.float32), ok)
    ...
    try:
        rms, k, d, rvecs, tvecs, flags = calibrate(
            cv2, np, objpoints, imgpoints, image_size, not args.skip_check_cond, initial_k, initial_d
        )
    except cv2.error:
        rms, k, d, rvecs, tvecs, flags = calibrate(
            cv2, np, objpoints, imgpoints, image_size, False, initial_k, initial_d
        )
