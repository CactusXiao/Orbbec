...
from decode_h265_to_jpg import resolve_video_fps
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="FPS for generated MP4 outputs. Defaults to the captured session rate.",
    )
...
def main() -> int:
    ...
    try:
        video_fps, video_fps_source = resolve_video_fps(
            args.video_fps,
            session_dir,
            session_dir / "video.h265",
        )
    except ValueError as exc:
        print(f"[process_session] ERROR: {exc}", file=sys.stderr)
        return 1

    print(f"[process_session] video fps: {video_fps} ({video_fps_source})")
