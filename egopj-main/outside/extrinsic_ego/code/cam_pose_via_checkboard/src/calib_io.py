...
def _extract_intrinsics(
    entry: dict, camera_id: str, camera_model: str
    ...
    if camera_model == "fisheye":
        dist_vec = np.array([float(dist.get(f"k{i}", 0.0)) for i in range(1, 5)], dtype=np.float64)
    else:
        dist_vec = np.array(
            [
...
def load_calibrations(
    ...
    fixed_camera_ids = set(fixed_camera_ids)
    for cam_id in camera_ids:
        ...
        configured_model = fixed_camera_model if cam_id in fixed_camera_ids else target_camera_model
        camera_model = str(entry.get("camera_model", entry.get("model", configured_model))).lower()
        if camera_model not in {"pinhole", "fisheye"}:
            raise ValueError(f"Unsupported camera model {camera_model!r} for camera {cam_id}")
        K, dist, size = _extract_intrinsics(entry, cam_id, camera_model)
