...
def detect_apriltag_markers(image: np.ndarray, tag_family: str) -> tuple[list[tuple[int, np.ndarray]], str]:
    ...
    for corner, tag_id_arr in zip(corners_list, ids):
        tag_id = int(np.asarray(tag_id_arr).reshape(-1)[0])
...
def solve_single_tag_pnp(
    ...
    pnp_image_pts, pnp_K, pnp_dist = prepare_pnp_image_points(image_pts, K, dist, camera_model)
    ...
    rmse = reprojection_rmse(object_pts, image_pts, rvec, tvec, K, dist, camera_model)
...
def solve_camera_pose_from_tag_map(
    ...
    rmse = reprojection_rmse(
        object_pts[inlier_indices], image_pts[inlier_indices], rvec, tvec, K, dist, camera_model
    )
