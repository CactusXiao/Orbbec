...
from .exporters import (
    ...
    write_ego_extrinsics_json,
    write_frame_matrices,
    write_plots,
    write_trajectory_flat,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Estimate T_world_from_ego from chessboard/AprilTag observations")
...
def main() -> None:
    ...
    calib_json, calibrations = load_calibrations(
        ...
        fixed_camera_model=cfg.fixed_camera_model,
        target_camera_model=cfg.target_camera_model,
    )
    ...
    write_trajectory_flat(output_root / f"trajectory_tw_c{cfg.target_camera_id}.txt", rows)
    write_trajectory_json(output_root / "trajectory.json", rows)
    write_ego_extrinsics_json(output_root / "ego_extrinsics.json", rows)
