...
row-order alignment only when the frame counts match exactly.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Decode outside/stream_server_ubuntu session video.h265 into JPG frames and MP4 video."
    )
    parser.add_argument(
        "--session-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for decoded JPGs. Defaults to session-dir/decoded_jpg.",
    )
    parser.add_argument(
        "--output-video",
        type=Path,
    ...
    parser.add_argument(
        ...
        help=(
            "Frame rate used for MP4 output. H.265 elementary streams often do not carry reliable FPS. "
            "Default: 30."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--no-overwrite",
        action="store_true",
        help="Do not clear an existing output directory.",
    )
...
def resolve_ffmpeg_executable(ffmpeg_arg: str) -> str:
    ...
    try:
        ...
        raise FileNotFoundError(
            ...
            "Install FFmpeg or pass --ffmpeg /usr/bin/ffmpeg."
        ) from exc
...
def resolve_inputs(
    ...
    if session_dir is not None:
        ...
        output_dir = (args.output_dir or session_dir / DEFAULT_OUTPUT_DIR_NAME).resolve()
    else:
        video_path = args.video.resolve()
        metadata_path = args.metadata.resolve() if args.metadata else None
        output_dir = (args.output_dir or video_path.parent / DEFAULT_OUTPUT_DIR_NAME).resolve()
    ...
    if output_dir.exists():
        if not overwrite and any(output_dir.iterdir()):
            raise FileExistsError(
                f"Output directory is not empty: {output_dir}. Remove it or omit --no-overwrite."
            )
...
def run_ffmpeg_write_video(
    ...
    command = base_command + codec_args + [str(output_video_path)]
...
def read_encoded_frame_indices(network_log_path: Optional[Path]) -> Optional[list[int]]:
    """Read completed encoded frames in display order from network_log.jsonl.

    MediaCodec configuration and partial-buffer samples are not logical video
    frames. Complete samples are ordered by PTS when every sample has one;
    frame_index is the reliable fallback for older or mixed-schema logs.
    """
    ...
    with network_log_path.open("r", encoding="utf-8-sig") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            try:
                ...
                # Interrupted captures can leave a truncated final log line.
                # Only the final non-empty record is safe to ignore.
            ...
            if frame_index in completed_frames:
                existing_presentation_time = completed_frames[frame_index]
                if (
                    existing_presentation_time is not None
                ...
                if existing_presentation_time is None and presentation_time_us is not None:
...
def validate_network_log_mapping(
    ...
    count_error = client_encoder_count_alignment_error(
        session_dir,
...
def metadata_encoder_submission_state(metadata: dict[str, str]) -> str:
    """Classify whether a metadata row was handed to MediaCodec."""
    submitted = False
    not_submitted = False
...
def validate_frame_alignment(
    ...
    if encoded_frame_indices is None:
        if require_metadata and len(image_paths) != len(metadata_rows):
            raise ValueError(
                f"decoded JPG count {len(image_paths)} != metadata row count "
                f"{len(metadata_rows)}; network_log.jsonl is unavailable, so exact "
                "frame_index alignment cannot be verified"
            )
        return

    if len(set(encoded_frame_indices)) != len(encoded_frame_indices):
        raise ValueError("network_log.jsonl contains duplicate logical frame indices")
    if len(image_paths) != len(encoded_frame_indices):
        raise ValueError(
            f"decoded JPG count {len(image_paths)} != encoded frame index count "
    ...
    with index_path.open("w", encoding="utf-8", newline="") as handle:
        ...
        for decoded_index, image_path in enumerate(image_paths):
            ...
            if uses_frame_index_alignment:
                frame_index = encoded_frame_indices[decoded_index]
                metadata_entry = metadata_by_frame.get(frame_index)
...
def write_summary(
    ...
    exact_alignment = encoded_frame_indices is not None
    metadata_by_frame = metadata_rows_by_frame_index(metadata_rows)
    alignment_analysis = (
        analyze_metadata_frame_alignment(metadata_rows, encoded_frame_indices)
        if encoded_frame_indices is not None
        else None
    )
    matched_metadata_rows = (
        sum(frame_index in metadata_by_frame for frame_index in encoded_frame_indices)
        if encoded_frame_indices is not None
        else None
    )
    summary = {
        ...
        "decoded_index_csv": str(index_path),
        "jpg_quality_qv": jpg_quality,
        "video_fps": video_fps,
        "video_codec": video_codec,
        "decoded_frame_count": len(image_paths),
        "metadata_row_count": len(metadata_rows),
        "frame_count_matches_metadata": (
            len(image_paths) == len(metadata_rows)
            if metadata_rows and not exact_alignment
            else None
        ),
        "metadata_alignment_mode": (
            "network_log.frame_index" if exact_alignment else "metadata_row_order"
        ),
        "network_log_jsonl": str(network_log_path) if network_log_path else "",
        "encoded_frame_index_count": (
            len(encoded_frame_indices) if encoded_frame_indices is not None else None
        ),
        "metadata_rows_matched_to_encoded_frames": matched_metadata_rows,
        "metadata_rows_without_encoded_frames": (
            len(metadata_rows) - matched_metadata_rows
            if matched_metadata_rows is not None
            else None
        ),
        "allowed_unsubmitted_metadata_row_count": (
            len(alignment_analysis["allowed_unsubmitted"])
            if alignment_analysis is not None
            else None
        ),
        "allowed_unsubmitted_frame_indices_preview": (
            alignment_analysis["allowed_unsubmitted"][:20]
            if alignment_analysis is not None
            else []
        ),
        "submitted_metadata_rows_without_hevc_count": (
            len(alignment_analysis["submitted_without_hevc"])
            if alignment_analysis is not None
            else None
        ),
...
def main() -> int:
    ...
    try:
        ...
        prepare_output_dir(output_dir, overwrite=not args.no_overwrite)
        ...
        index_path = write_index_csv(
            ...
            require_metadata=metadata_path is not None,
        )
    ...
    print(f"[decode_h265_to_jpg] video fps: {args.video_fps}")
    print(f"[decode_h265_to_jpg] video codec: {args.video_codec}")
    print(f"[decode_h265_to_jpg] decoded frames: {len(image_paths)}")
