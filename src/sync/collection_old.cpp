...
public:
    ...
    bool start(const CollectionConfigUi &ui) {
        ...
        if(fisheyeEnabled_) {
            const auto fisheyeDevices = listAvailableFisheyeDevices();
    ...
    bool beginRecord(const fs::path &saveRoot, const std::string &subjectId, const std::string &taskName, int episodeN) {
        ...
        local.passthroughRgbMjpg = isJpegLikeExt(cfg_.save.colorExt) && !local.needRgbForColorCloud;
...
private:
    ...
    void startCoordinatorThread() {
        ...
        coordinatorThread_ = std::thread([this]() { coordinatorLoop(); });
    }
    ...
    bool tryFinalizeOneMultiviewSlotLocked() {
        ...
        for(const auto &sn: session_.deviceSns) {
            ...
            for(const auto t: session_.typesPerCamSave) {
                ...
                const fs::path outPath = session_.dest / buf.camKey / dataTypeLabel(t)
                                         / (frameIndex + ((t == CollectDataType::RGB) ? colorExtNormalized(cfg_.save.colorExt) : ".png"));
                if(t == CollectDataType::RGB) {
