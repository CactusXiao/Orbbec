...
采集程序登录后的菜单中，点击 **Web Workbench**（手型标定右侧）即可在主机默认浏览器中打开 Label/QC 网页工作台，采集菜单保持可操作。默认地址为 `http://127.0.0.1:18882/`，使用主机已有的网页工作台服务。

如需使用其他已部署的工作台，可在采集配置中设置 `frontends.webWorkbench.url`，例如：

```json
{"frontends": {"webWorkbench": {"url": "http://127.0.0.1:18882/"}}}
```

网页沿用独立的账号登录流程。更新可执行文件后，重新启动采集程序即可看到入口。
